...
DYNAMIC API SYSTEM: Each request gets a FRESH, unique api_id paired with
the master api_hash. API credentials are generated per-request by the backend
(get-next-task / get-batch-tasks) and passed in the task payload.
NO API REUSE - every message/task uses a different api_id.
"""
...
- API credentials are passed in the task payload (not stored on account)
- NO API REUSE - every message uses a unique api_id
- Master api_hash is constant, api_id is random per-request

CRITICAL RULES:
- NO ACCOUNT RUNS WITHOUT PROXY AND FINGERPRINT!
- API credentials come from task payload (generated per-request)
...
async def _get_or_create_client_internal(account: dict, setup_handler=None, task_proxy: dict = None,
    ...
    try:
        # DYNAMIC API: Each request gets FRESH api_id/api_hash from the task payload
        # The backend generates unique credentials per-request - never reuse!
        api_id = account.get("api_id")
        api_hash = account.get("api_hash")
        
        # API credentials are now generated per-request by the backend
        # They should always be present in the account data from get-next-task/get-batch-tasks
        if not api_id or not api_hash:
            print(f"  ⛔ [SKIP] {phone} - NO API CREDENTIALS IN TASK PAYLOAD")
            print(f"          → Backend should provide fresh api_id/api_hash per request")
            return None
        
        print(f"  ✓ [API] Using fresh credentials: {api_id[:4]}...{api_id[-2:]}")
