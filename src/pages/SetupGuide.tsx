...
`;
  ...
  // ========== MASTER RUNNER (Single file that does EVERYTHING) ==========
            ...
            if sender_username:
                ...
                for variant in [f"@{username_clean}", username_clean]:
                    response = await http.get(
                        f"{SUPABASE_URL_BASE}/rest/v1/conversations",
                        headers={"apikey": SUPABASE_KEY, "Authorization": f"Bearer {SUPABASE_KEY}"},
                        params={"account_id": f"eq.{account_id}", "recipient_username": f"ilike.{variant}", "first_message_sent": "eq.true", "select": "id"}
                    )
                    if response.status_code == 200 and response.json():
                        return True
            if sender_phone:
                ...
                for pv in [f"+{digits}", digits, sender_phone]:
                    response = await http.get(
                        f"{SUPABASE_URL_BASE}/rest/v1/conversations",
                        headers={"apikey": SUPABASE_KEY, "Authorization": f"Bearer {SUPABASE_KEY}"},
                        params={"account_id": f"eq.{account_id}", "recipient_phone": f"eq.{pv}", "first_message_sent": "eq.true", "select": "id"}
                    )
                    if response.status_code == 200 and response.json():
                        return True
            return False
...
async def setup_livechat_handler(client, account_id):
    @client.on(events.NewMessage(incoming=True))
    async def handler(event):
        try:
            try:
...
async def change_profile_photo(client, photo_source):
    try:
        from telethon.tl.functions.photos import UploadProfilePhotoRequest
        import aiohttp
        temp_path = os.path.join(SESSION_FOLDER, "temp_photo.jpg")
    ...
    try:
        ...
        if not user and recipient_username:
            try:
                ...
                pass
        if not user:
            contact = InputPhoneContact(client_id=random.randint(0, 999999), phone=recipient_phone, first_name=recipient_first_name or "Friend", last_name="")
            result = await client(ImportContactsRequest([contact]))
            if result.users:
                user = result.users[0]
        if not user:
...
async def add_contact(client, phone, first_name, last_name=""):
    try:
        ...
        result = await client(ImportContactsRequest([contact]))
        return True, phone, None if result.imported else "Exists"
    except Exception as e:
        return False, phone, str(e)
...
async def process_campaign_task(task):
    ...
    account_phone = account.get("phone_number", "????")[-4:]
    if not account_id or not recipient:
        return {"success": False, "error": "Missing data", "campaign_recipient_id": msg.get("campaign_recipient_id"), "account_id": account_id}
    try:
        client = await get_or_create_client(account, task_proxy=proxy)
        if not client:
            return {"success": False, "error": "No connection", "campaign_recipient_id": msg.get("campaign_recipient_id"), "account_id": account_id}
        await asyncio.sleep(random.uniform(0.5, 3))
        print(f"  [{account_phone}] -> {recipient}")
        success, error = await send_message(client, recipient, content, msg.get("media_url"))
        api_creds = account.get("telegram_api_credentials")
        api_credential_id = api_creds.get("id") if api_creds else account.get("api_credential_id")
        result = {"success": success, "error": error, "campaign_recipient_id": msg.get("campaign_recipient_id"), "message_id": msg.get("id"), "account_id": account_id, "api_credential_id": api_credential_id, "content": content, "recipient_phone": recipient, "recipient_name": task.get("recipient_name")}
        if error and "privacy" in error.lower():
            result["skip_account"] = True
            result["retry_with_different_account"] = True
        print(f"    {'OK' if success else 'FAIL: ' + str(error)[:30]}")
        return result
    except Exception as e:
...
async def process_warmup_task(task):
    ...
    try:
        ...
        if not client:
            await report_result("warmup_chat", {"task_id": task_id, "pair_id": pair_id, "account_id": account.get("id"), "success": False, "error": "No connection"})
            return {"task_id": task_id, "success": False}
        if task_type == "warmup_add_contact":
            target_phone = task_data.get("phone") or task_data.get("recipient_phone")
            success, _, error = await add_contact(client, target_phone, task_data.get("first_name", "Friend"))
            ...
            success, _, error = await join_channel(client, task_data.get("channel_username") or task.get("channel_username"))
            await report_result("warmup", {"task_id": task_id, "task_type": "join_channel", "account_id": account.get("id"), "success": success, "error": error})
            return {"task_id": task_id, "success": success}
        elif task_type == "warmup_view_content":
            success, _, error = await view_channel_messages(client, task_data.get("channel_username") or task.get("channel_username"))
            await report_result("warmup", {"task_id": task_id, "task_type": "view_content", "account_id": account.get("id"), "success": success, "error": error})
            return {"task_id": task_id, "success": success}
        elif task_type == "warmup_send_reaction":
            success, _, error = await send_reaction(client, task_data.get("channel_username") or task.get("channel_username"))
...
async def campaign_loop():
    ...
    while RUNNING:
        try:
            ...
            print(f"  [CAMPAIGN] Done: {success_count}/{len(results)}")
            if delay_after > 0:
                await asyncio.sleep(delay_after)
        except Exception as e:
        ...
        try:
            ...
            if task_type == "wait":
                ...
                if new_accounts:
                    ...
                    await asyncio.gather(*[get_or_create_client(acc, setup_handler=setup_livechat_handler, task_proxy=acc.get("proxy")) for acc in new_accounts], return_exceptions=True)
                    for acc in new_accounts:
                        if acc.get("id"):
...
async def warmup_loop():
    ...
    while RUNNING:
        try:
            ...
            success_count = sum(1 for r in results if isinstance(r, dict) and r.get("success"))
            print(f"  [WARMUP] Done: {success_count}/{len(results)}")
            await asyncio.sleep(delay_after)
        except Exception as e:
...
async def main():
    ...
    print("=" * 60)
    print("  Running 5 task types in a SINGLE process:")
    print("    - Campaigns")
    print("    - LiveChat")
    ...
    print("=" * 60)
    print()
    await asyncio.gather(campaign_loop(), livechat_loop(), account_loop(), warmup_loop(), block_loop(), return_exceptions=True)
    print("\\nShutting down...")
    await shutdown_all()
    print("Goodbye!")
...
`;
  ...
  // ========== RUN.BAT (Simple - runs master runner) ==========
...
echo  [2/2] Starting Master Runner...
echo         (Handles ALL tasks in one process)
echo.
...
`;
  ...
  const downloadZip = async () => {
    ...
    toast.success("ZIP downloaded! Master runner included.");
  };
  ...
  const downloadVpsZip = async () => {
    ...
    toast.success("VPS ZIP downloaded! Run main_runner.py on your server.");
  };
              ...
              <CardContent className="p-8 text-center space-y-6">
                ...
                <div className="text-left bg-muted rounded-lg p-4 space-y-3">
                  ...
                  <ul className="list-disc list-inside text-sm text-muted-foreground space-y-1">
                    ...
                    <li><code className="text-blue-500">main_runner.py</code> - <strong>Master runner (ALL tasks in ONE file)</strong></li>
                  ...
                  <p className="text-xs text-green-600 dark:text-green-400 mt-2">✓ Connects to each account ONCE • Single process • Faster & simpler</p>
                </div>
