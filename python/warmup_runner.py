...
from client_manager import (
    ...
    shutdown_all, release_client, active_clients
)
...
async def process_single_task(task: dict) -> dict:
    ...
    NOTE: Warmup must NOT keep session DBs open for long periods.
    We disconnect + release the client after each task to avoid blocking
    livechat/campaign from opening the same session in another process.
    """
    ...
    try:
        ...
        if task_type == "warmup_add_contact":
            ...
            await report_result(
                "warmup_chat",
                {
                    ...
                    "success": success,
                    "error": error,
                    "message_type": "add_contact",  # tells backend to mark contacts_exchanged=true
                    "is_cycle_last": is_cycle_last,
                },
            )
        ...
        # Release the SQLite session file by disconnecting the client.
...
async def process_regular_warmup_task(task: dict):
    ...
    NOTE: Disconnect after each task so warmup does not keep the session DB open.
    """
