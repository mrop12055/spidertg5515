...
serve(async (req) => {
  ...
  try {
    ...
    // UNIFIED RUNNER: Get all task types combined (campaign, livechat, account)
    if (runner === "unified") {
      const unifiedBatchSize = Math.min(batch_size || 50, 50);
      const tasks: any[] = [];
      const usedAccountIds = new Set<string>();

      // Helper to format account with proxy for unified runner
      const formatAccountWithProxy = (account: any) => {
        const apiCred = account.telegram_api_credentials;
      ...
      // 1. Get pending campaign recipients (priority - no delay in unified)
      const { data: runningCampaigns } = await supabase
        .from("campaigns")
        .select("id, message_template, seat_id, name")
        .eq("status", "running");

      if (runningCampaigns && runningCampaigns.length > 0) {
        const campaignIds = runningCampaigns.map(c => c.id);
        const campaignLookup = new Map(runningCampaigns.map(c => [c.id, { message_template: c.message_template, seat_id: c.seat_id, name: c.name }]));
        ...
        if (pendingRecipients && pendingRecipients.length > 0) {
          ...
          for (const recipient of pendingRecipients) {
            ...
            const account = usableAccounts.find((a: any) => !usedAccountIds.has(a.id));
            if (!account) break;

            const campaign = campaignLookup.get(recipient.campaign_id);
      ...
      // 2. Get pending livechat messages
      if (tasks.length < unifiedBatchSize) {
        const { data: pendingMessages } = await supabase
          ...
          .select(`
            *,
            conversations!inner(
              id, account_id, recipient_phone, recipient_name, recipient_telegram_id, seat_id
            )
          `)
          .eq("status", "pending")
          .eq("direction", "outgoing")
      ...
      // 3. Get account management tasks
      if (tasks.length < unifiedBatchSize) {
        const { data: accountTasks } = await supabase
          ...
          .eq("status", "pending")
          .in("task_type", [
            "spambot_check", "change_name", "change_photo", 
            "sync_profile", "verify_session", "logout_sessions"
          ])
          .order("created_at", { ascending: true })
          .limit(unifiedBatchSize - tasks.length);
        ...
        if (accountTasks && accountTasks.length > 0) {
          ...
          for (const task of accountTasks as any[]) {
            if (tasks.length >= unifiedBatchSize) break;
            ...
            // Parse task_data from result field if exists
            let taskData = {};
            if (task.result) {
            ...
            tasks.push({
              ...
              task_id: task.id,
              task_data: taskData,
              account: formatAccountWithProxy(accountData),
            });
          }
      ...
      console.log(`[get-batch-tasks] UNIFIED: Returning ${tasks.length} tasks`);
