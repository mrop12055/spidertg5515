...
// API rate limiting - 45 messages per API per 24 hours
const API_DAILY_LIMIT = 45;
...
serve(async (req) => {
  ...
  try {
    ...
    // ========== LIVECHAT RECEIVER: Only returns accounts for listening ==========
    if (runner === "livechat_receiver") {
      // Heartbeat
      supabase
        .from("runner_heartbeats")
        .upsert({ runner_name: "livechat_receiver", last_seen: new Date().toISOString(), status: "online" }, { onConflict: "runner_name" })
        .then(() => {});
      ...
      console.log(`[get-next-task] LiveChat Receiver: returning ${validAccounts.length} accounts for listening`);
    ...
    // ========== LIVECHAT SENDER: Only returns pending send tasks ==========
    if (runner === "livechat_sender") {
      // Heartbeat
      supabase
        .from("runner_heartbeats")
        .upsert({ runner_name: "livechat_sender", last_seen: new Date().toISOString(), status: "online" }, { onConflict: "runner_name" })
        .then(() => {});

      // Check for pending live chat messages (non-campaign)
      ...
      if (pendingMessages && pendingMessages.length > 0) {
        for (const msg of pendingMessages) {
          ...
          if (account && proxy?.status === "active") {
            ...
            console.log(`[get-next-task] LiveChat Sender: msg ${msg.id.slice(0, 8)} (priority=${msg.priority})`);
    ...
    // ========== LEGACY LIVECHAT (backwards compatibility) ==========
    if (runner === "livechat") {
